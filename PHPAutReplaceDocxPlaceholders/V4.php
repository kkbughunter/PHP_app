 ...
 * and handle table row placeholders, adding new rows as needed. Writes to a new output file.
 *
...
/**
 * Process table rows to replace placeholders and add new rows if needed.
 */
function processTableRows(DOMElement $table, array $replacements, DOMXPath $xp, DOMDocument $doc): void
{
    ...
    foreach ($rowArray as $rowIndex => $row) {
        ...
        if ($hasPlaceholders) {
            // Identify the placeholder key (e.g., 'arg_t1' from '%*arg_t1*%')
            $firstPlaceholder = reset($placeholders)[0] ?? '';
            $varName = preg_replace('/%*|\*/', '', $firstPlaceholder);
            // Extract the base key (e.g., 'arg_t' from 'arg_t1')
            if (preg_match('/^(arg_t)\d+$/', $varName, $matches)) {
                $baseKey = $matches[1] . '(i)';
                if (isset($replacements[$baseKey]) && is_array($replacements[$baseKey]) && is_array($replacements[$baseKey][0])) {
                        ...
                        foreach ($cells as $cellIndex => $cell) {
                            if (isset($placeholders[$cellIndex])) {
                                foreach ($placeholders[$cellIndex] as $placeholder) {
                                    ...
                                    $colIndex = (int)preg_replace('/^arg_t/', '', $colVarName) - 1;
                    ...
                    // Add new rows for remaining data
                    foreach ($tableData as $rowData) {
                        $newRowXml = '<w:tr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">';
                        foreach ($rowData as $cellValue) {
                            $newRowXml .= '<w:tc>';
                            $newRowXml .= '<w:tcPr><w:tcW w:w="0" w:type="auto"/></w:tcPr>';
                            $newRowXml .= '<w:p><w:r><w:t>' . htmlspecialchars((string)$cellValue, ENT_QUOTES, 'UTF-8') . '</w:t></w:r></w:p>';
                            $newRowXml .= '</w:tc>';
                        }
                        $newRowXml .= '</w:tr>';
...
/**
 * Generate WordprocessingML for a full table.
 */
